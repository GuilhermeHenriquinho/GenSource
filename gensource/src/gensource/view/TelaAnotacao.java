		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (Objects.nonNull(anotacoes) && !anotacoes.isEmpty()) {
					...
					JOptionPane.showMessageDialog(null, "Anotações para o Atributo \"" + getAtributo().getNomeAtributo()
							+ "\" salvas com Sucesso!");
					dispose();
				} else {
				...
				if (Objects.nonNull(am)) {
					...
					if (am.getTitulo().contains("Column") && !am.getTitulo().contains("Join")) {
						...
						if (Objects.nonNull(screen.getAnotacao()) && screen.getAnotacao().length() > 0) {
							...
							anotacao.setAnotacao(screen.getAnotacao());
							for (int i = 0; i < anotacoesString.size(); i++) {
								if (anotacoesString.get(i).getAnotacao().contains("@Column")) {
									anotacoesString.remove(i);
									break;
								}
							}
					...
					//Transient
					if(am.getTitulo().contains("Transient")) {
				        int resposta = JOptionPane.showConfirmDialog(null, "Deseja confirmar a adição da anotação Transient?", "Confirmação", JOptionPane.YES_NO_OPTION);
					...
					// Relacionamentos
					if (getAtributo().getIsRelacionamento() && am.getTitulo().contains("OneToMany")
							|| am.getTitulo().contains("ManyToOne") || am.getTitulo().contains("OneToOne")) {
						TelaRelacionamentos screen = new TelaRelacionamentos();
						screen.setRelation("@" + am.getTitulo());
						screen.setVisible(true);
						if (Objects.nonNull(screen.getAnotacao()) && screen.getAnotacao().length() > 0) {
							AnotacaoString anotacao = new AnotacaoString();
							anotacao.setAnotacao(screen.getAnotacao());
							for (int i = 0; i < anotacoesString.size(); i++) {
								if (anotacoesString.get(i).getAnotacao().contains("@" + am.getTitulo())) {
									anotacoesString.remove(i);
									break;
								}
							}
